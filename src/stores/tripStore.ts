...
import { GUEST_TRIPS } from '@/data/demoData'
  ...
  persist(
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        ...
        // Guest catalog is empty (or guest-safe samples only). Strip any
        // previously cached USA/Holland/… family seeds so old tabs and the
        // brother's persist key cannot keep showing them after deploy.
        if (isGuestTripStore()) {
          ...
          state.trips = normalizePersistedTripFields(injected.trips)
        ...
        // Timestamp / collection normalize does not need family JSON and must
        // run before the first merge — a leftover 2099 updatedAt would reject
        // the spouse's cloud edits. Itinerary one-shots wait on the lazy chunk.
        state.trips = normalizePersistedTripFields(state.trips)
        void import('@/lib/repairLiveSeedTrips').then(({ repairLiveSeedTrips }) => {
          if (isGuestTripStore()) return
          useTripStore.setState(current => ({
            trips: repairLiveSeedTrips(current.trips),
          }))
        })
 ...
 * cloud data. Signing out always resets the in-memory/guest list to empty so
 * a previous account's USA trip cannot reappear from the old shared
 * `myk-trip-plan-store` key, and so guests never get the family seed catalog.
 */
export async function switchTripStoreAccount(userId: string | null): Promise<void> {
  ...
  if (plan.resetTo === 'guest-empty' || plan.resetTo === 'empty') {
