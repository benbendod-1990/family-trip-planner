...
describe('trip front poster is data-driven for every seed', () => {
  ...
  it('USA front looks like the Florida itinerary, not a geo sketch', () => {
    ...
    assert.match(blob, /epcot/i)
    assert.match(blob, /cococay|perfect day/i)
    assert.match(blob, /utopia/i)
    assert.match(blob, /seaworld|peppa|gatorland|disney springs/i)
    assert.equal(/animal kingdom/i.test(blob), false)
    assert.equal(/miami beach/i.test(blob), false)
    assert.equal(/חוף/.test(blob), false)
    assert.ok(poster.stays.length >= 3)
    assert.ok(poster.stays.some(s => /solterra/i.test(s.name)))
    assert.equal(poster.stays.some(s => /holiday inn/i.test(s.name)), false)
    assert.ok(poster.stops.length >= 10)
    assert.ok(poster.stops.length <= usa.days.length)
