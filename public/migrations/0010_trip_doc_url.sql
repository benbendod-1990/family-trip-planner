-- Persist the linked Google Doc on public.trips.
--
-- hydrateTrip used to omit docUrl because the table had no column. A signed-in
-- member (especially an invitee whose persist cache starts empty) then lost
-- the planning-Doc link after cloud pull, and Trip Doc hid "מסמך התכנון".
--
-- This migration:
--   * adds doc_url / doc_title
--   * backfills the USA Mar-2027 and Holland seed Docs
--   * teaches save_trip() to write them (coalesce on update so an older client
--     that omits the keys cannot wipe a backfill)
--
...
update public.trips
set
  doc_url = coalesce(nullif(doc_url, ''), 'https://docs.google.com/document/d/1gAabX9m9vWPLBjvQZszXpEh5_rCJWC6dArhMe8E8aGo/edit'),
  doc_title = coalesce(nullif(doc_title, ''), '★ ארה״ב 2027 — תכנון קבוצה')
where id = 'b38fc010-9096-45c9-b8df-191e369143dc';

update public.trips
set
  doc_url = coalesce(nullif(doc_url, ''), 'https://docs.google.com/document/d/1H22xt-Q6VrHNQ4PMvF0UvSw7K5XBvVc5VzZgfyEeyjc/edit?tab=t.0')
where id = '34980c90-bd66-4270-8d45-3e96787b07ef';
...
begin
  ...
  on conflict (id) do update set
    ...
    -- Keep a previously backfilled / user-set Doc when the payload omits it
    -- (old clients, or a phone that hydrated before the seed restore ran).
