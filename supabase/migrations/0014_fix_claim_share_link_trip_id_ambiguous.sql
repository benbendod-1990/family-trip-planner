...
-- `ON CONFLICT (trip_id, user_id)` then clashes with that OUT param
-- (Postgres 42702).
--
-- Live production was hotfixed the same way as
-- fix_claim_share_link_trip_id_ambiguous: name the PK constraint instead
-- of the columns. This file is the repo copy so a fresh SQL Editor paste
-- matches production, and so environments that already ran 0012/0013 pick
-- up the fix.
--
-- Idempotent: CREATE OR REPLACE + grants only. Safe to paste more than
-- once. Merging this file does NOT apply it to production. Run it in
-- Supabase only if the live function still uses ON CONFLICT (trip_id, user_id).
