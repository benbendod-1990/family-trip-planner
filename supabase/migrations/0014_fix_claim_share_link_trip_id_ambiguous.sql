...
--
-- PL/pgSQL RETURNS TABLE (trip_id uuid, already_member boolean) exposes
-- `trip_id` as an OUT variable in the function body. Unqualified
-- `ON CONFLICT (trip_id, user_id)` then clashes with trip_members.trip_id.
--
-- Live production was hotfixed as fix_claim_share_link_trip_id_ambiguous:
-- `ON CONFLICT ON CONSTRAINT trip_members_pkey`, plus revoke execute from
-- public/anon and grant execute to authenticated only. This file is the
-- repo copy so a fresh SQL Editor paste matches production, and so
-- environments that already ran 0012/0013 pick up the fix.
--
-- Idempotent: CREATE OR REPLACE only. Safe to paste more than once.
-- Merging this file does NOT apply it to production. Production already
-- has the hotfix; paste only if another environment still has
